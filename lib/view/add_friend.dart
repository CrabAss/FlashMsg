...
class AddFriendScreenState extends State<AddFriendScreen> {
  ...
  void findFriend(String userURI) async {
    if (!userURI.startsWith(userURIPrefix)) {
      setState(() {
        isLoading = false;
        _showAlertDialog("This QR code is not generated by $appName.");
      });
    } else {
      ...
      if (result.documents.length > 0) {
        ...
        if (result.documents.length == 0) {
          setState(() {
            isLoading = false;
            _showAlertDialog("This user does not exist.");
          });
        } else {
          setState(() {
            isLoading = false;
  ...
  void addNewFriend(DocumentSnapshot userProfile) async {
    await Firestore.instance
        .collection('users').document(myAccount.id)
        .collection('friends').document(userProfile['id'])
        .setData({
      'id': userProfile['id'],
    });
    await Firestore.instance
        .collection('users').document(userProfile['id'])
        .collection('friends').document(myAccount.id)
        .setData({
      'id': myAccount.id,
    });
    setState(() {
